...
- Docker от 24.0 + Docker Compose v2
...
> Дата в имени файла должна точно совпадать с `SNAPSHOT_DATE` в `.env`.
...
2. `backend` — собирается образ запускается `python db/loader.py` (загрузка CSV в БД, 350k строк транзакций) запускается `uvicorn`
...
# 4. Следить за загрузкой
docker compose logs backend -f
# Ищите строку: ЗАГРУЗКА ЗАВЕРШЕНА УСПЕШНО
```

> `docker compose down -v` удаляет все данные PostgreSQL — это необходимо,
...
| Метод | Путь | Описание |
|---|---|---|
| GET | `/api/clients` | Список hot_customers с фильтрами |
...
> **Ограничение Render бесплатного тарифа:** volume mounts для CSV файлов не поддерживаются. Для демо — либо включить CSV в Docker образ, либо предзагрузить данные в БД локально и использовать `pg_dump/restore`.
...
## Что не входит в задачи сервиса
...
- Realtime обновление — только батч при старте контейнера
