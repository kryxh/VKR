...
def show_client_list():
    ...
    for item in items:
        ...
        cols[2].write(f"{SEGMENT_EMOJI.get(seg, '⚪')} {seg.capitalize()}")
...
def show_client_card(customer_id: str):
    ...
    seg_label = f"{SEGMENT_EMOJI.get(seg, '⚪')} {seg.capitalize()}"
    ...
    with right_col:
        ...
        if not recs:
            ...
            if all_same_just:
                j     = justifications[0]
                icon  = JUSTIFICATION_ICON.get(j, "📌")
                jtext = JUSTIFICATION_MAP.get(j, j)
                st.caption(f"{icon} **Основание для всех рекомендаций:** {jtext}")
            ...
            for rec in recs:
                ...
                # Теги с подписями внутри плашек — только заполненные
                tag_parts = []
                if category:
                ...
                if not all_same_just:
                    caption_parts.append(f"{icon} {jtext}")
